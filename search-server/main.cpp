...
public:
    ...
    void AddDocument(int document_id, const string& document) {
        ...
        double tf = 1.0 / words.size(); // доля одного слова в строке
        ...
        for (const string& word : query_words.minus_words) {
            if (word_to_document_freqs_.count(word) != 0) {
                for (const auto& i : word_to_document_freqs_.at(word)) {
                    numbers_to_delete.push_back(i.first);
                }
            }
        }
        for (const auto& num : numbers_to_delete) {
            document_to_relevance.erase(num);
        }

        for (const auto& dtr : document_to_relevance) {
            matched_documents.push_back({ dtr.first, dtr.second });
        }
        return matched_documents;
    }
